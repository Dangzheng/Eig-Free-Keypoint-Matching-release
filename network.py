...
class MyNetwork(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("Matchnet", reuse=tf.AUTO_REUSE):
            ...
            x_rms1 = root2 / tf.reduce_mean(tf.sqrt(tf.reduce_sum(x_mss[:, :, :, :2], axis=-1, keepdims=True))
                                            , axis=2, keepdims=True)
            x_rms2 = root2 / tf.reduce_mean(tf.sqrt(tf.reduce_sum(x_mss[:, :, :, 2:], axis=-1, keepdims=True))
                                            , axis=2, keepdims=True)
            ...
            self.p1 = tf.reshape(tf.stack([
                scale1, zeros, dx1, zeros, scale1, dy1, zeros, zeros, ones
            ], axis=1), (x_shp[0], 3, 3))
            self.p2 = tf.reshape(tf.stack([
                scale2, zeros, dx2, zeros, scale2, dy2, zeros, zeros, ones
            ], axis=1), (x_shp[0], 3, 3))
            xx = tf.transpose(tf.reshape(x_norm, (x_shp[0], x_shp[2], 4)), (0, 2, 1))
    ...
    def _build_loss(self):
        ...
        with tf.variable_scope("EigFreeLoss", reuse=tf.AUTO_REUSE):
            ...
            # build loss
            p2_e_gt = tf.matmul(tf.transpose(tf.matrix_inverse(self.p2), (0, 2, 1)),
                                tf.reshape(e_gt, (x_shp[0], 3, 3)))
            e_gt = tf.reshape(tf.matmul(p2_e_gt, tf.matrix_inverse(self.p1)), (x_shp[0], 9))
            ...
            p_hat = tf.eye(9, batch_shape=[x_shp[0]], dtype=tf.float32) - tf.matmul(p, p_t)
    ...
    def test(self, data):
        ...
        for test_mode in test_mode_list:
            test_process(
                ...
                self.is_training,
                None, None, None,
                self.logits, self.loss, data[test_mode],
